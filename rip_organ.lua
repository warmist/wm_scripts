...
--ripping part:
local body=trg.body
body.components.body_part_status[organ_id].missing=true
local wid=body.wound_next_id
body.wound_next_id=wid+1
body.wounds:insert("#",
	{new=true,id=wid,flags={severed_part=true--[[,mortal_wound=true]]},
    	parts={
...
function assignBody(item,unit,id_missing)
  ...
  int32_t size_cur;
    ...
    body.size_modifier=unit.appearance.size_modifier
    body.body_part_relsize:resize(#uBody.body_plan.body_parts) 
    for i=0,#uBody.body_plan.body_parts-1 do
        body.body_part_relsize[i]=uBody.body_plan.body_parts[i].relsize --or from enemy...
    end
